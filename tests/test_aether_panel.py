...
""",
    ...
    def test_main_has_no_preflight_or_hotkeys(self) -> None:
        keys = {k for _, k, _ in MAIN_ACTIONS}
        self.assertNotIn("preflight_next", keys)
        self.assertNotIn("preflight", keys)
        for _label, _k, hot in MAIN_ACTIONS:
            self.assertEqual(hot, "", msg="main board must have no letter hotkeys")
        adv = {k for _, k, _ in ADVANCED_ACTIONS}
        self.assertIn("preflight_next", adv)
