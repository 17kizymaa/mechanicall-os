#!/bin/sh
# Smoke Send-folder overlay. Not Yes. Not FILES. USB ≠ LTE. Not Play.
...
grep -q "SitHit.Send -> openSendOverlay" "$NAV"
grep -qv "sendProject" "$NAV"
grep -q "sendOverlay" "$RACK"
grep -q "tap outside to dismiss" "$RACK"
grep -q "SitHit.Files" "$NAV"
echo "sit-send-smoke: source PASS (overlay, not yes, not sendProject, FILES other hit)"

EDGE="${POCKET_EDGE:-anphuni@100.70.86.90}"
SERIAL="${POCKET_SERIAL:-RZCW2038KHN}"
OUT="${1:-$ROOT/dev/50_ss-closing-apk/03_module-panels/output/send-overlay.png}"
