#!/usr/bin/env python3
"""SDK module stills for sit-glass-polish. Never Confirm. Never A33."""
    ...
    while time.time() - t0 < timeout:
        try:
            ...
            boot = subprocess.check_output(
                [str(ADB), "-s", s, "shell", "getprop", "sys.boot_completed"]
            ).decode().strip()
            if boot == "1":
                return
        except Exception:
...
def main() -> int:
    ...
    time.sleep(1.2)
    shot("04-bind-picker")
    tap_ui("sit-sdk-pocket") or tap_ui("sit-pocket")
    time.sleep(0.7)
    ...
    idx = ["# SDK module stills — sit-glass-polish", "", "**Not Yes. Not A33. Not LTE. Not Play.**", "", f"serial `{ser}` · wm {sw}x{sh} · APK 0.19.2-module-panels", "", "| file |", "|------|"]
    for n in names:
        idx.append(f"| `{n}` |")
    idx += ["", "Confirm was not tapped. Opening the emulator is not Yes.", ""]
